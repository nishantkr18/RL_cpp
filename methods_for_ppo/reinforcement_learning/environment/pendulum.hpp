...
class Pendulum
{
 public:
  ...
  class State
  {
   public:
    ...
    double& Theta() { return theta; }
        ...
        //! Get the theta.
    double SinTheta() const { return data[0]; }
    //! Modify the value of theta.
    double& SinTheta() { return data[0]; }

        //! Get the theta.
    double CosTheta() const { return data[1]; }
    //! Modify the value of theta.
    double& CosTheta() { return data[1]; }
   ...
   */
  Pendulum(const double maxAngularVelocity = 8.0,
                ...
                State& nextState)
  {
    ...
    nextState.AngularVelocity() = math::ClampRange(newAngularVelocity,
        -maxAngularVelocity, maxAngularVelocity);
    nextState.SinTheta() = std::sin(nextState.Theta());
    nextState.CosTheta() = std::cos(nextState.Theta());
  ...
  State InitialSample()
  {
    ...
    state.SinTheta() = std::sin(state.Theta());
    state.CosTheta() = std::cos(state.Theta());
