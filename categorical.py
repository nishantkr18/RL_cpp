...
class DQNAgent:
    ...
    def update_model(self) -> torch.Tensor:
        ...
        with torch.no_grad():
            lossGradients = torch.zeros(size=dists.size())
            for i in range(self.batch_size):
                lossGradients[i, action[i]] = - (proj_dist[i] / (1e-10 + dists[i, action[i]]))


        self.optimizer.zero_grad()
        dists.backward(lossGradients)
        self.optimizer.step()
    ...
    def train(self, num_frames: int, plotting_interval: int = 5000):
        ...
        for frame_idx in range(1, num_frames + 1):
            ...
            if done:
                state = self.env.reset()
                scores.append(score)
                print(frame_idx, "\tavg score:", sum(scores)/len(scores), "\tepisodes:", len(scores))
                score = 0
